...
<jsp:useBean id='dataSourceBean'  class='studiranje.ip.database.bean.ViaServiceUserListBean' scope='session'></jsp:useBean>

<%@ taglib uri = "http://java.sun.com/jsp/jstl/core" prefix = "c"%>
<%@ taglib uri = "http://java.sun.com/jsp/jstl/sql" prefix = "sql"%>

<%@page import = "studiranje.ip.bean.PageBean" %>
         <c:set var='total' value="${dataSourceBean.count(pageContext.session, usersPageBean)}"></c:set>
		...
		<c:if test="${usersPageBean.pageNo gt 0}">	        
	        <table class='viewport_table simpletable'>
	        	...
	        	<tbody class='simpletable'>
			        <c:forEach var = "row" items = "${dataSourceBean.getAllUsers(pageContext.session, usersPageBean)}">
		         		<tr class='simpletable'>
		         			<td class='simpletable'><c:out value="${row.user.username}"></c:out></td>
		         			<td class='simpletable'><c:out value="${row.user.firstname}"></c:out></td>
		         			<td class='simpletable'><c:out value="${row.user.secondname}"></c:out></td>
		         			<td class='simpletable'><c:out value="${row.user.email}"></c:out></td>
		         			<td class='simpletable'><c:out value="${row.getRequisit().telephone}"></c:out></td>
		         			<td class='simpletable'><c:out value="${row.getRequisit().city}"></c:out></td>
		         			<td class='simpletable'><c:out value="${row.getRequisit().country}"></c:out></td>
		         		</tr>
		         	</c:forEach>
         		</tbody>
         	</table>
