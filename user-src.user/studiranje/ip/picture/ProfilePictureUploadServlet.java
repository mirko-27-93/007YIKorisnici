...
/**
 * Сервлет који се користи за постављање профилне слике. 
 */
@WebServlet("/ProfilePictureUploadServlet")
public class ProfilePictureUploadServlet extends HttpServlet {
	private static final long serialVersionUID = -7988619415133111599L;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(contentType.indexOf("multipart/form-data")>=0) {
			...
			try {
				...
				if(count!=1) {
					controller.getMessages().setInfoBean(infoBean).setUploadProfilePictureGeneralFailureForWeb(request, response);
				...
				while(i.hasNext()) {
					...
					if(!fi.isFormField()) {
						...
						controller.archiveProfileImage(username, fileName, bais, request.getSession());
						infoBean.reset();
					}
				}
